...
function AlertRow({ alert }) {
  ...
  return (
    <div style={{
      display: "grid",
      gridTemplateColumns: "70px 90px 140px 120px 110px 1fr",
      gap: 10,
      padding: "8px 14px",
    ...
    >
      <span style={{ color: "#444", fontSize: 11 }}>{fmt(alert.timestamp)}</span>
      <SeverityBadge sev={alert.severity} />
      <span style={{ color: "#a0a0a0" }}>{alert.rule_id}</span>
      <span style={{ color: "#e0e0e0" }}>{alert.src_ip}</span>
      <span style={{ color: "#666" }}>{alert.dst_ip}:{alert.dst_port || "—"}</span>
      <span style={{ color: "#888", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        {alert.title || alert.description} <span style={{ color: "#555" }}>×{alert.signal_count || 1}</span>
      </span>
    </div>
  );
}
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontSize: 11, color: "#444", marginLeft: 8 }}>
            {paused ? "PAUSED" : (useMock ? "DEMO MODE" : "LIVE")}
          </span>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 16 }}>
          ...
          <div style={{ background: "#0d0d0d", border: "1px solid #1e1e1e", borderRadius: 6, overflow: "hidden" }}>
            ...
            <div style={{
              ...
              <span>Time</span><span>Severity</span><span>Rule</span>
              <span>Source IP</span><span>Destination</span><span>Description</span>
            </div>
